                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-5">
                    {projects.map((info) => (
                        <div key={info.id} className="mx-auto bg-[#1A1A1A] flex flex-col items-center w-full p-4 rounded-lg border border-zinc-600 hover:border-blue-500 duration-300">
                            ...
                            <Link href={`/${locale}/${info.urlName}`} className='text-stone-200 text-left w-full font-bold text-lg md:text-2xl flex gap-2 items-center'><p>{info.name}</p><Image src={externalLink} alt="Link" className="h-4 md:h-5 w-auto"/></Link>
